...
- Mobile adaptation (iPhone/MacBook Safari): viewport with `maximum-scale=1.0, user-scalable=no, viewport-fit=cover` + iOS web-app meta tags (`apple-mobile-web-app-capable`, `apple-mobile-web-app-title`, `theme-color`) for home-screen fullscreen launch. Layout breakpoint `@media (max-width: 760px)`: spiral takes full width, task panel below it (border-top instead of border-left); wide screens keep the side panel. All touch targets ≥ 44 px (including `.lib-item` buttons). Anti-scroll during finger drag: `touch-action: none` on `#spiral`/`.lib-item`, `preventDefault` in the document-level `pointermove` (`{ passive: false }`), `overscroll-behavior: none` and `overflow-x: hidden` on body, `-webkit-tap-highlight-color: transparent` on interactive elements; fallback `touchmove`+`preventDefault` on `#spiral`/`#lib-list` only when `window.PointerEvent` is missing (iOS < 13).
