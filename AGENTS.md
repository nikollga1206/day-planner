...
- SVG rendering (`renderSchedule`, `renderTaskLabels`, `renderTimeLabels`). The seam at 04:00 is drawn as a visible radial step (R_INNER→R_OUTER) on top of the segments. Task labels are straight text rotated by (mid-angle − 90°), flipped if upside down; font auto-fit ladder: default → smaller + condensed (`textLength`) → emoji only (narrow segments, ~15 min or less): the task emoji is drawn large at the segment center, with the full name and time in a `<title>` tooltip on the segment (hover) and in the edit panel (tap). Time labels (hour marks + task boundaries) are placed strictly OUTSIDE the circle in two radial tiers with leader lines — near tier for inner-disk times, far tier for outer-ring times; their font is smaller than task labels, with a white halo (`paint-order: stroke`).
